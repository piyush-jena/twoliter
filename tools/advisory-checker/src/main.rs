...
use models::{Advisory, Epoch, PackageName, PackageVersionManifest, EV, Args};
...
const ADVISORY_CHECKER_ANCHOR_DATE: &str = "2025-06-11";
...
fn collect_advisories(dir: &Path) -> Result<HashMap<PackageName, Vec<Advisory>>> {
    let mut advisories_by_product: HashMap<PackageName, Vec<Advisory>> = HashMap::new();

    fn scan(dir: &Path, map: &mut HashMap<PackageName, Vec<Advisory>>) -> Result<()> {
        for entry in fs::read_dir(dir).context(ReadDirSnafu { path: dir })? {
            let path = entry.context(ReadDirEntrySnafu)?.path();
            if path.is_dir() {
                scan(&path, map)?;
            } else if path.extension() == Some(OsStr::new("toml")) {
                let content = fs::read_to_string(&path).context(ReadFileSnafu { path: &path })?;
                let advisory: Advisory = toml::from_str(&content).context(ParseAdvisorySnafu { path: &path })?;
                for product in &advisory.advisory_info.products {
                    map.entry(product.package_name.clone()).or_default().push(
                        toml::from_str(&content).context(ParseAdvisorySnafu { path: &path })?
                    );
                }
            }
        }
    ...
    scan(dir, &mut advisories_by_product)?;
    ...
    for (package_name, mut advisories) in advisories_by_product {
        ...
        if let Some(spec_ev) = pkg_metadata.get(&package_name) {
            ...
            if let Some(latest) = advisories.last() {
                ...
                if issue_date.as_str() > ADVISORY_CHECKER_ANCHOR_DATE && !latest.advisory_info.end_of_life {
                    violations.push(format!(
...
fn main() -> Result<()> {
    ...
    println!("Advisory validation passed for {}", args.advisories_dir.display());
    Ok(())
}
