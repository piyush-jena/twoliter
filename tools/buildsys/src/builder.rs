...
impl DockerBuild {
    ...
    pub(crate) fn repack_variant(args: RepackVariantArgs, manifest: &Manifest) -> Result<Self> {
        ...
        match manifest.info().image_format() {
            ...
            // UKI repack is supported. `img2img` extracts the FAT boot
            // partition with mtools, rebuilds `EFI/Linux/bottlerocket.efi` from
            ...
            Some(ImageFormat::Raw | ImageFormat::Qcow2 | ImageFormat::Vmdk) | None => {}
        }
